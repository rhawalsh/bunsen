DOCUMENTATION = """
  lookup: special_released_repo_roots
  author: Joe Shimkus <jshimkus@redhat.com>
  version_added: "0.1"
  short_description: return special-case uris for released
                      distribution/architecture pairings
  description:
    - This lookup returns the special-case released uris for specified
      distribution/architecture pairings.
  arguments:
...
class LookupModule(LookupBase):
  def run(self, _terms, variables = None, **kwargs):
      ...
      for (distribution, arch) in pairings:
        roots.extend(
            Distribution.makeItem(distribution,
                                  architecture = arch).specialRepoRoots)
      if len(roots) == 0:
        roots = ["none"]
      return roots
